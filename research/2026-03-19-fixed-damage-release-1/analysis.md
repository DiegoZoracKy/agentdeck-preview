...
- Haiku's inversion has a clear policy mechanism in the behavioral layer: both Haiku cells scored `1.0` on `position_policy_delta`, meaning the action distribution changed maximally by position in shared visible states. Concretely, at `80 HP / 3 potions`, Haiku healed as first player and attacked as second player.
