#!/usr/bin/env python3
"""Minimal matrix-based experiment runner template."""
...
def _build_game(cell: Dict[str, Any], manifest: Dict[str, Any]):
    game_name = (manifest.get("game") or {}).get("name")
    ...
    raise ValueError(
        "Template runner supports FixedDamageGame and VariableDamageGame by default. "
        "Customize _build_game() for other games."
    )
...
def _list_cells(matrix: Dict[str, Any]) -> None:
    for cell in matrix.get("cells", []):
        print(f"{cell['id']} [{cell.get('phase', '?')}] - {cell.get('question', '')}")
