#!/usr/bin/env python3
"""Run VariableDamage Reinforcement 1 cells using AgentDeck-native configuration only."""
...
from agentdeck import AgentDeck, AgentDeckConfig, ConclusionPolicy, VariableDamageGame
...
def _build_player(
    ...
    *,
    ...
    if kind == "bot":
        bot_cls = _bot_from_class(player_spec["class"])
        return bot_cls(**common_kwargs)
...
def _build_game(cell: Dict[str, Any], manifest: Dict[str, Any]) -> VariableDamageGame:
    game_config = dict((manifest.get("game") or {}).get("config") or {})
    game_config.update(cell.get("game") or {})
    return VariableDamageGame(**game_config)
...
def main() -> None:
    ...
    parser.add_argument("--phase", help="Run all cells in one phase (e.g. P0, P1)")
    parser.add_argument(
        "--cell", action="append", dest="cells", help="Run one or more specific cell IDs"
    )
