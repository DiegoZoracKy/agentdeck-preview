...
- **Current-schema design**: replay targets Recorder v2.0 artifacts and the current in-memory `MatchResult` surface.
        ...
        *,
        ...
        Args:
            match_result: MatchResult object or dict (from Recorder.load_match()).
                         MUST conform to Recorder v2.0 schema expectations.
            scheduler: Playback scheduler (defaults to ReplayScheduler()).

        Raises:
            ValueError: If schema_version missing or != 2.0
        """
...
**Guarantees**: MUST deserialize Recorder v2.0 artifacts. MUST hydrate events and metadata. MUST initialize isolated EventBus. MUST raise ValueError if schema invalid.
...
**Contract**:
- Accept: MatchResult or dict from `Recorder.load_match()` conforming to Recorder schema v2.0, optional ReplayScheduler
...
- Raise: ValueError if `schema_version` missing or not exactly Recorder v2.0
...
- MUST: Emit prompt/interaction metadata from the recorded event payloads without reshaping (handshake, gameplay, conclusion, parse_failure)

## 6. Invariants & Guarantees

### 6.1 Input Normalization (IN)
1. **IN1**: MUST accept Recorder v2.0 artifacts only (raise `ValueError` if `schema_version` missing or not exactly `2.0`).
...
17. **PM1**: MUST deliver gameplay `interaction.prompt_text`, `interaction.prompt_blocks`, and `interaction.response_text` from the recorded event payload to spectators.
18. **PM2**: MUST preserve lifecycle and parse-failure prompt metadata exactly as recorded.
19. **PM3**: MUST treat the recorded event payload as canonical and MUST NOT synthesize, flatten, or recompute prompt/interaction data.

### 6.7 State Tracking (ST)
20. **ST1**: MUST preserve recorded `state_before` / `state_after` payloads exactly as stored in canonical gameplay events.
...
2. Validate `schema_version` is exactly Recorder v2.0 (raise `ValueError` otherwise).
3. Deserialize events into Event objects without reshaping their payloads.
...
- PLAYER_HANDSHAKE_* → Recorded events with lifecycle prompt fields preserved
...
- PLAYER_CONCLUSION → Recorded events with lifecycle prompt fields preserved

## 8. Error Handling & Edge Cases

**Input Validation**:
- Missing `schema_version` or any version other than `2.0` → **MUST raise `ValueError`** with a descriptive message.
...
class PromptMetadataAnalyzer:
    ...
    def on_gameplay(self, event: Event) -> None:
        ...
        if interaction.get('prompt_text'):
            print(f"[TURN {ctx['phase_index']}] Prompt: {len(interaction['prompt_text'])} chars")
...
class PromptViewer:
    ...
    def on_player_handshake_complete(self, event: Event) -> None:
        self.exchanges.append({
            ...
            "prompt": event.data.get('prompt_text', ''),
    ...
    def on_gameplay(self, event: Event) -> None:
        ...
        if interaction:
            self.exchanges.append({
                ...
                "prompt": interaction.get('prompt_text', ''),
                "response": interaction.get('response_text', ''),
            })
...
|-------|------------|-------------------|
| Input normalization | IN1-IN3 | Feed v2.0 artifacts (dict and MatchResult). Assert schema validation. Verify ValueError on wrong schema. |
...
| Prompt / interaction metadata | PM1-PM3 | Inspect recorded event payloads. Replay. Verify lifecycle prompt fields and gameplay `interaction` fields are delivered to spectators unchanged. |
...
1. Load recording with canonical prompt/interaction payloads
2. Replay with prompt metadata analyzer
3. Verify every event carrying prompt or interaction data is replayed with identical contents
...
| **Prompt / interaction replay** | Keeping LLM I/O metadata inside recorded events preserves a single timeline while enabling prompt UX research and A/B testing analysis. |
...
| **Clean slate (Recorder v2.0 only)** | No backward compatibility shims keeps implementation simple, testable, and maintainable. Historical recordings can be migrated offline when needed. |
...
- `SPEC-GAMEPLAY-EVENT-DATA.md` (canonical gameplay interaction payload)
- `SPEC-RECORDER.md` §6.7 (canonical prompt/interaction persistence)
...
This specification defines the current replay contract for complete lifecycle support (handshake, turns, conclusion, parse failures) using Recorder v2.0 canonical event payloads as the source of truth.
