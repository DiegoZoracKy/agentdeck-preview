...
- Follows `SPEC.md` §2.4 reproducibility mandates: every match captures seed, configuration, environment, and game settings (including `information_level`, `allowed_actions`, and game params) for exact replay.
...
- **Game Configuration Capture**: Record game settings (`information_level`, `allowed_actions`, `params`) and persist player configuration snapshots (model, controller, key parameters) via `player_summaries` for reproducibility and provenance.
...
- **Schema Versioning**: Tag recordings with schema version (`1.4` for match recordings with game params, `1.0` for batches) and enforce version checks in `load_match()` for forward compatibility.
...
### Recorder(output_dir="records", *, session=None, collectors=None, schema_version="1.4")
...
- `schema_version`: Schema version tag (default `"1.4"` for match recordings with game params and enriched prompt metadata events).
...
- Extracts game configuration from `Game.describe()` (`information_level`, `allowed_actions`, `params`).
...
- Records game configuration (`information_level`, `allowed_actions`, `params`).
...
- Captures controller parsing failures (schema v1.3+, current v1.4).
...
7. **SV1**: MUST tag all recordings with `schema_version` field (currently `"1.4"` for match recordings with game params, `"1.0"` for batch recordings) and `schema_type` field (`"match"` or `"batch"`).
...
13. **MC4**: MUST capture game configuration via `Game.describe()`: `name`, `module`, `allowed_actions`, optional `information_level`, and `params` (JSON-serialisable game config).
...
**Status**: Finalized for schema v1.4.0.
...
**Alignment**: Player lifecycle events are fully defined in SPEC-OBSERVABILITY v1.2.0 §3.1.1-3.1.2. This spec defines the event serialization contract (prompt payload embedding, parse-failure capture) for Recorder schema v1.3+ (current v1.4). See SPEC-REPLAY for how replays reconstruct lifecycle events from the enriched `events` stream.
...
print(f"Game Params: {match_data['metadata']['game_config'].get('params')}")
...
- ✅ Game configuration (`information_level`, `allowed_actions`, `params`) (MC4)
...
| Metadata completeness | MC1-MC5 | Inspect match/batch files. Assert all required metadata fields present (including `information_level`, `allowed_actions`, `params`, template sources). |
...
- **Game configuration capture**: Recording `information_level`, `allowed_actions`, and `params` enables faithful replay and A/B test analysis.
...
- `SPEC-GAME.md` v0.8.0 (Game configuration, `information_level`, `allowed_actions`, params export)
