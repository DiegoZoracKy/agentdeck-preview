...
>
> **Changes in v0.8.0**:
> - Add `describe()` + `get_config_params()` for exporting game configuration
> - Require JSON-serialisable `params` in recorder-facing metadata
...
- Expose static game configuration for recorder metadata and replay tooling.
...
- Role: Export static game configuration for recorder metadata and reproducibility.
- Return: JSON-serialisable dict describing game identity and configuration.
- MUST: Include keys `name`, `module`, `allowed_actions`, and `params`.
- SHOULD: Include `information_level` when the game defines it.
- MUST NOT: Include runtime state (turn counters, RNG, current scores).
- MUST: Be deterministic for a given game instance.

### get_config_params() -> Dict[str, Any]
- Role: Provide game-specific configuration parameters (implementation-defined).
- Return: JSON-serialisable dict of static parameters (defaults, tuning knobs, limits).
- Default: `{}` when not overridden.
- MUST: Only include static configuration (no runtime state or derived values).
...
### 5.12 Game Configuration Export (GC) — *New in v0.8.0*
42. **GC1**: `describe()` MUST return JSON-serialisable output containing `name`, `module`, `allowed_actions`, and `params`.
43. **GC2**: `get_config_params()` MUST return JSON-serialisable static configuration only (no runtime state).
44. **GC3**: `describe()` output MUST be deterministic for the same game instance (no time-based or random values).
...
| Game config export | GC1-GC3 | Call `describe()` on sample games; assert JSON-serialisable output and stable keys. |
