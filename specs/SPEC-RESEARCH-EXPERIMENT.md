...
- **RE2**: When using `research_validate.py`, `results.json` and `results.csv` MUST have been generated by `research_export.py` (provenance check).
...
- **RE8**: Export scripts MUST produce deterministic output for identical recordings, excluding `generated_at` timestamps. Use `--deterministic` flag or exclude timestamps from diff-sensitive checks.
