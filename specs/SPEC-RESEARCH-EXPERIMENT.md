...
- **RE1**: Every experiment directory MUST contain `README.md` and `manifest.yaml`.
- **RE2**: `results.json` and `results.csv` MUST be generated by `research_export.py`.
- **RE3**: `manifest.yaml` MUST include all required fields in §4.2.
- **RE4**: `experiment_id` MUST match the experiment folder name.
- **RE5**: `schema_version` MUST be an integer >= 1 for manifests and results.
- **RE6**: Raw recordings MUST NOT be committed; `recordings/` contains pointers only.
- **RE7**: `research/INDEX.md` MUST match the output of `research_index.py`.
- **RE8**: Export scripts MUST be deterministic for a given recordings set.
