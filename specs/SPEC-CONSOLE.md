...
30. **M1**: MUST populate `MatchResult.metadata` with game name, player names, duration, turn count, and truncation info. `turns` MUST equal the number of structural `GAMEPLAY` events recorded for the completed match, including one event per participant decision in a simultaneous round. Console MUST derive this value from the captured event stream rather than trust mechanic-owned or internal state counters.
