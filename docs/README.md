# AgentDeck Documentation

This directory contains the user-facing documentation that sits between the
root README and the component specifications.

## Start Here

- [Repository README](../README.md) - product overview, quick start, and flagship evidence
- [How to Run a Study](how-to-run-a-study.md) - end-to-end research package workflow
- [Research Directory](../research/README.md) - experiment package layout and index
- [Specification Hub](../specs/SPEC.md) - source of truth for engine contracts
- [Contributing Guide](../CONTRIBUTING.md) - development workflow, local setup, and review checklist
- [Viewer README](../viewer/README.md) - browser replay viewer usage
- [Security Policy](../SECURITY.md) - vulnerability reporting process
- [Release Checklist](release-checklist.md) - operational checklist for the next package release
- [Release Notes](releases/0.2.0.md) - current public package release notes

## Additional Notes

- [Raw Run Recovery Plan](raw-run-recovery-plan.md)
- [Research Rerun Comparison](research-rerun-comparison.md)

The docs are intentionally lean. Component-level behavior belongs in `specs/`;
experiment-specific interpretation belongs in `research/<experiment>/analysis/`.
